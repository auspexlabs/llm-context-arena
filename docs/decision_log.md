...
- **date:** 2026-07-13 · **status:** open · **triggered_by:** Curia-on-Curia MCP dogfood turn `f78e96e6-bbfa-4cbb-8fdd-5b5a0e3ef842` · **docs_updated:** `docs/decision_log.md`, `backend/turn_service.py`, `backend/cost_tracking.py`, `tests/unit/test_cost_tracking.py`, `tests/unit/test_turn_service.py` · **related:** `DEC-015`, `DEC-017`, `DEC-018`
...
- **decision:** Conversation JSON remains the canonical record. Maintain a recoverable SQLite session catalog as a query projection, updated through the storage mutation boundary with atomic JSON replacement, per-conversation locking, write intents, monotonic source revisions, and checksums covering the conversation plus turn sidecars. Reconcile the projection at startup and expose keyset-paginated, server-filtered results through `/api/sessions`; retain `/api/conversations` as a compatibility view over the same service.
